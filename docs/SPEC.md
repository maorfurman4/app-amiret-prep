...
- שורה עליונה: `StreakBadge` (להבה + מספר ימים, מוסתר כשאין streak; בגלילת עלייה הספרות מתגלגלות עם דופק spring, ובאבני דרך זוהר חד־פעמי), `UserMenu`, `ThemeToggle`.
...
- `StreakCelebration` — הודעה קומפקטית ולא חוסמת **פעם ביום ורק לאחר פעילות היום** (מפתח `amiret_streak_celebration_seen_date`, יום לפי Asia/Jerusalem), מספר מתגלגל, דופק וזוהר חד־פעמי; נסגרת אוטומטית אחרי 4.2 שניות, בכפתור או Escape. אבני דרך: 7, 14, 30 וכל 30 ימים לאחר מכן.
- סגירת כל הטבעות: מילוי 900ms, סימון סיום וזוהר מתפוגג, ללא לולאה; חזרה לדף עם טבעות סגורות נחגגת פעם ביום (`amiret_ring_celebration_seen_date`). כל התנועה מכבדת reduced-motion. תצוגת פיתוח בלבד: `/dev/streak-gamification`, ללא שינוי נתוני למידה.
...
### עדכון חוויית הרצף — 2026-09-24
לאחר אישור המחשה ויזואלית, חגיגת הרצף הוחלפה בחלון מרכזי עם להבת ניאון תכולה־סגולה. משך 3.8 שניות לאחר טעינת התמונה: צבירת אנרגיה, התפרצות, רגיעה וסגירה. שלבים: ימים 1–6 ניצוץ; 7–29 להבה; 30+ אנרגיה עם הילות וגיצים נוספים. התמונה נשמרת ב־`public/images/gamification/neon-flame.png`. החלון משתמש ב־dialog מקורי עם ניהול מיקוד, Escape וכפתור סגירה, ומכבד reduced-motion. תנאי ההופעה נשארו פעילות היום ופעם ביום. בתצוגת הפיתוח אפשר לבחור ימים ולהחזיק את העיצוב פתוח לבדיקה.
