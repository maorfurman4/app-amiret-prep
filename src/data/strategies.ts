...
export const GAME_RULES: { icon: LucideIcon; title: string; body: string }[] = [
  {
    ...
    body:
      ...
      'נבחן שנבהל מקושי עולה ומתחיל לפעול פחות בביטחון, פוגע בעצמו פעמיים: גם בציון וגם בריכוז. הציונים הגבוהים ביותר (120+) מגיעים דרך פרקים קשים, ולכן קושי עולה הוא בדיוק המסלול שאתה רוצה להיות בו.',
  },
  {
    icon: DoorClosed,
    ...
    body:
      ...
      'המסקנה המעשית: לפני שאתה עוזב פרק, בלחיצה או כי הזמן עומד להיגמר, עשה סבב אחרון מהיר לוודא שלכל שאלה יש תשובה כלשהי מסומנת, גם אם היא ניחוש.',
  },
  {
    icon: Timer,
...
export const TIME_INTRO =
  'בגלל שהטיימר קשיח והזמן לא עובר בין פרקים (כמו שראית בנושא "חוקי המשחק"), תזמון הוא לא עניין של משמעת אישית. הוא חלק מהמבנה של המבחן עצמו. ' +
  'ההבדל בין נבחן טוב לבינוני הוא לא רק כמה אנגלית הוא יודע. הנבחן הטוב מחליט מראש, עוד לפני שהוא רואה שאלה, כמה זמן מותר לו "להיתקע" על שאלה מכל סוג, ועומד בהחלטה הזו גם כשהוא תקוע ומתוסכל. ' +
...
export const RESTATEMENT_GUARDS: LayeredRule[] = [
  {
    ...
    why: 'הכמת קובע על כמה מהנושא הטענה חלה. מסיח טיפוסי משאיר את כל המשפט כמו שהוא ומחליף רק את הכמת: "רוב" הופך ל"כולם", "מעטים" ל"אף אחד". מילה נרדפת לכמת (most = the majority) כשרה, אבל כמת בעוצמה אחרת לא.',
    example: {
      kind: 'pair',
  ...
  },
  {
    ...
    example: {
      ...
      note: 'may (אפשרות) הפך ל-will (ודאות). אותו נושא ואותו כיוון, אבל עוצמה אחרת.',
...
export const TOO_SIMILAR_RULE: LayeredRule = {
  id: 'too-similar',
  title: 'דמיון למקור',
  rule: 'תשובה שדומה מאוד למקור היא סימן לבדוק, לא סיבה לפסול.',
...
export const CHANGE_ANSWER_RULE: LayeredRule = {
  ...
  example: {
    kind: 'scenario',
    text: 'בסבב השני אתה רואה שבחרת "will reduce" כשהמקור אמר "may reduce". זו סיבה קונקרטית (שומר המודאליות), אז משנים. לעומת זאת, אם תשובה אחרת פשוט "נשמעת עכשיו טוב יותר" ולא מצאת מה פסול בזו שבחרת, נשארים.',
  },
...
export const QUESTION_GUIDES: QuestionGuide[] = [
  {
    ...
    workedExample: {
      ...
      correctIndex: 1,
      walkthrough: [
        'מילת קישור: "Although" = ניגוד. יש שבח ("praised"), ולכן מה שבא אחריו חייב להסתייג ממנו, לא להמשיך אותו.',
        'השלמה עצמאית בראש: "היא ביקשה מהצוות לעשות משהו לכמה סעיפים לפני ההגשה". סביר שמדובר בתיקון או בשיפור, לא בפעולה קיצונית.',
        'פסילה: "ignore" (להתעלם) לא הגיוני, כי למה לבקש להתעלם מסעיפים לפני הגשה סופית? "publish" (לפרסם) לא מתאים, כי מדובר בהגשה, לא בפרסום. "discard" (לזרוק) קיצוני מדי מול "praised" (היא ציינה שבח, לא פסילה מוחלטת).',
        '"revise" (לתקן, לערוך) היא היחידה שמתיישבת עם הניגוד העדין של "although": שבח כללי, אבל עדיין יש מה לשפר לפני הסוף. זו התשובה.',
      ],
    },
    deep: {
      ...
      method: {
        ...
        steps: [
          ...
          },
          {
            ...
            body: 'אם תקרא את ארבע האפשרויות לפני שגיבשת ציפייה משלך, המוח "ייצמד" למילה שנשמעת הכי מוכרת, גם אם היא שגויה. ניחוש משלך, אפילו כללי ("צריך כאן משהו שלילי"), מגן עליך מהמלכודת הזו: יש לך נקודת ייחוס, ואתה משווה אליה כל אפשרות.',
          },
          {
            title: 'הגדר מה הפער צריך לעשות',
            body: 'השאלה היא לא "איזה סוג מילה" (זה כמעט תמיד נתון, כי כל האפשרויות מאותו סוג), אלא "איזה תפקיד": פעולה של תיקון? תיאור של כמות? תוצאה שלילית? ככל שההגדרה שלך חדה יותר, כך קל יותר לפסול אפשרות שנשמעת טוב אבל עושה משהו אחר.',
          },
          {
            title: 'סלק לפי משמעות',
            body: 'הצב כל אפשרות בתוך המשפט, לא לבד. מילה יכולה להיות תקינה כשלעצמה ועדיין לא להתאים להקשר הזה, ולכן בודקים תמיד בתוך המשפט המלא. שים לב גם למבנה שאחרי הפער: יש מילים ש"מתחברות" רק למילת יחס מסוימת.',
          },
          {
            title: 'בדוק התאמת טון',
            body: 'זהו הסינון האחרון והכי אמין כשנשארות שתי אפשרויות קרובות: המשפט כולו חיובי, שלילי או ניטרלי? מילת הקישור (ראה מדריך מילות הקישור) כבר סימנה לך את הטון הצפוי. אם התשובה סותרת אותו, היא שגויה גם אם היא "נשמעת" סבירה.',
          },
        ],
      },
      tips: {
        ...
        items: [
          ...
          },
          {
            ...
            example:
              '"The new law is designed to _____ small businesses from unfair competition." — ארבעת הפעלים (protect / support / promote / encourage) כולם מתאימים ל"עזרה לעסקים קטנים", אבל רק "protect" מתחבר ל-"from": מגנים על X מפני Y. המבנה שאחרי הפער הוא לפעמים מה שמכריע בין שתי אפשרויות קרובות במשמעות.',
          },
          {
          ...
          {
            tip: 'אם אין לך מושג, בחר לפי יחסי הניגוד במשפט',
            example:
          ...
          },
          {
            ...
            example:
              'un-/dis-/mis- = שלילה או טעות, re- = שוב, over- = יותר מדי, under- = פחות מדי. מילה זרה לגמרי עדיין "מסגירה" כך את כיוון המשמעות שלה (חיובי או שלילי, יותר או פחות), ולעיתים זה מספיק כדי לפסול אפשרויות לפי הטון, בלי לדעת את התרגום המדויק.',
          },
        ],
      },
      traps: {
  ...
  },
  {
    ...
    approach: [
      ...
      { step: 'עבור מסיח-מסיח מול הגרעין', detail: 'לכל תשובה שאל: אותו מי? אותו מה? אותו כיוון? כל סטייה פוסלת את התשובה מיד.' },
    ...
    stuck: [
      ...
      { step: 'שתי תשובות נראות נכונות?', detail: 'אחת מהן כמעט תמיד סוטה באחד "השומרים". השווה אותן זו לזו (לא רק למקור) ומצא במה הן נבדלות. ההבדל הזה הוא בדיוק מה שהשאלה בודקת.' },
    ...
    workedExample: {
      ...
      walkthrough: [
        ...
        'שומר הכמת והשלילה: אין כאן כמת מיוחד לבדוק, אבל יש כיוון ברור. האזהרות לא מנעו את הפעולה.',
        'שומר הזמן והתוצאה: המקור אומר "continued operating" (המשיך לפעול), לא שהמפעל הפסיק. תשובה 2 ("stopped") הופכת את הכיוון לגמרי, ולכן נפסלת.',
        'תשובה 3 הופכת תפקידים (המפקח "אישר" במקום "הזהיר"), וזה סותר את "warnings" במקור. תשובה 4 ("improved") אומרת את ההפך הגמור מ-"continued... without".',
        'תשובה 1: "התעלם מהאזהרות והמשיך לעבוד עם אוורור לקוי": אותו מי, אותו מה, ואותו ניגוד בין האזהרה למעשה. זה הניסוח הנכון.',
      ],
    },
    deep: {
        ...
        steps: [
          {
            ...
            body: 'פרק את משפט המקור לשלושה רכיבים: **מי** (הנושא), **מה קרה** (הפעולה והתוצאה), ו-**איזה קשר לוגי** מחבר ביניהם: ניגוד (although, despite), סיבה-תוצאה (because, therefore) או תנאי (if, unless; ראה מדריך מילות הקישור). זו "תעודת הזהות" של המשפט: תשובה שמשנה אפילו רכיב אחד מהשלושה מתארת משפט אחר לגמרי, גם אם היא נשמעת דומה.',
          },
          {
            title: 'התמקד במשמעות, לא במילים',
            body: 'נסח לעצמך בעברית מה המשפט אומר, לפני שאתה קורא אפשרות אחת. הסיבה: ברגע שאתה משווה מילה-למילה נגד האנגלית המקורית, קל "להיתפס" על תשובה שחולקת אוצר מילים אבל לא לוגיקה. השוואה נגד תרגום עברי משלך מנטרלת את המלכודת הזו, כי אתה בודק רעיון מול רעיון ולא מילה מול מילה.',
          },
          {
            title: 'בדוק: האם כל חלקי המשמעות נשמרו?',
            body: 'משפטי מקור מכילים לרוב יותר ממידע אחד (למשל: מי + מתי + למה). תשובה נכונה שומרת על **כולם**. תשובה שמדייקת בחלק אחד ומחסירה או משנה חלק אחר היא עדיין שגויה. זו הסיבה ש"קרוב" לא מספיק כאן.',
          },
        ],
      },
      layered: [
      ...
      traps: {
        ...
        items: [
          ...
          },
          {
            ...
            detail: 'כמעט תמיד יש אפשרות שאומרת את ההפך המדויק מהמקור. אם נפלת עליה, זה סימן ברור שהבנת נכון את הנושא של המשפט אבל טעית בכיוון היחס הלוגי שבו. שווה לחזור ולבדוק את מילת הקישור.',
          },
          {
            trap: 'משמעות חלקית (partial meaning)',
            detail: 'התשובה נכונה לגבי חלק מהמשפט, אבל מחסירה מידע חשוב מהחלק השני. תמיד משווים נגד כל שלושת הרכיבים של הגרעין (מי, מה, קשר לוגי), לא רק מול הרכיב הראשון שקפץ לעין.',
          },
        ],
      },
      ctas: [
  ...
  },
  {
    ...
    approach: [
      ...
      { step: 'שמור את "הרעיון המרכזי" וההסקה לסוף', detail: 'אחרי שענית על הפרטים אתה כבר מכיר את הקטע לעומק, ושאלות הכלל נהיות קלות בהרבה.' },
      { step: 'פסול תשובות קיצוניות', detail: 'תשובות עם only / always / never / all כמעט תמיד שגויות, אלא אם הקטע אמר זאת במפורש. התשובה הנכונה בדרך כלל "מאוזנת".' },
    ],
    stuck: [
      ...
      { step: 'מילה קריטית לא מוכרת בשאלת אוצר מילים?', detail: 'קרא את המשפט שסביבה ושאל מה הגיוני שיהיה שם. אלו שאלות הקשר, לא מילון, והפירוש המילולי הוא לרוב המלכודת.' },
      CHANGE_ANSWER_STEP,
    ...
    workedExample: {
      ...
      correctIndex: 1,
      walkthrough: [
        'הצצה בשאלה קודם: מילת המפתח היא "urban beekeeping" ו"according to the passage", כלומר שאלת פרט ולא הסקה. מחפשים משפט ספציפי, לא רושם כללי.',
        'קריאת הקטע: המשפט השני מכיל את הניגוד (While = "בעוד ש"): יתרון (האבקה) מול סיכון (תחרות על פרחים, פגיעה בדבורים בר).',
        'תשובה 1 ("always... thrive"): מילת הקיצון "always" מול קטע שאומר "may... sometimes". נפסלת מיד, וזו בדיוק המלכודת של תשובות קיצוניות.',
        'תשובה 3: "banned" (נאסר) לא מופיע בקטע בכלל. זה פרט מומצא, לא מידע מהטקסט. גם "only for honey" שבתשובה 4 לא מופיע בקטע.',
    ...
    deep: {
      ...
      method: {
        ...
        intro:
          'שתי גישות קיצוניות נפוצות בשוק ההכנה: לקרוא את כל השאלות לפני הקטע (מסוכן, כי הקריאה הופכת ל"חיפוש" ומפספסת את הרעיון המרכזי), או לקרוא את הקטע במלואו בלי שום כיוון (מסוכן אחרת: קוראים בלי לדעת מה מחפשים, וחוזרים לטקסט שוב ושוב). ' +
        ...
        steps: [
          ...
          },
          {
            ...
            body: 'קריאה מהירה ורצופה, בלי לעצור על מילה לא מוכרת (סמן אותה בראש והמשך). המטרה כאן היא לצאת עם שני דברים: מה הרעיון המרכזי, ומה יש בכל פסקה בקווים כלליים. לא צריך לזכור כל משפט.',
          },
          {
          ...
          },
          {
            ...
            body: 'אחרי שענית על שאלות הפרט, אתה כבר מכיר את הקטע לעומק בלי מאמץ נוסף, ושאלות הכלל (main idea, הסקה) נהיות קלות הרבה יותר בשלב הזה מאשר אם היית מנסה לענות עליהן ראשונות.',
          },
        ],
      },
      questionKinds: {
        title: '3 סוגי שאלות, ולמה כל אחד דורש גישה שונה',
        intro:
      ...
      elimination: {
        ...
        intro: 'כשלא בטוחים, פוסלים ולא מנחשים באקראי. ארבעה סוגי תשובות חוזרים כמלכודות, ולזהות אותם זה כבר חצי מהעבודה:',
        items: [
          { flag: 'קיצוני מדי', desc: 'מילים כמו "always" / "never" / "all" / "completely". קטעים אקדמיים כמעט אף פעם לא טוענים טענות כה מוחלטות. התשובה הנכונה בדרך כלל מאוזנת ("often", "may", "some").' },
      ...
      ctas: [
        {
          ...
          icon: Compass,
          tone: 'sage',
          title: 'שיטות הקריאה בשוק, ולמה השיטה המשולבת עדיפה',
...
export const CONNECTOR_CATEGORIES: ConnectorCategory[] = [
  {
    ...
    words: [
      ...
      },
      {
        ...
        grammar:
          'הן שונות מ-although ב"עוצמה": הן לא בהכרח מציגות משהו "רע" מול "טוב", אלא סתם מדגישות הבדל בין שני דברים מקבילים. שימושיות מאוד בשאלות שמשוות בין שתי קבוצות, שני זמנים, או שני מקומות.',
...
export const INVEST_POINTS: { icon: LucideIcon; title: string; body: string }[] = [
  ...
  },
  {
    ...
    body:
      ...
      'האתר עדיין לא מדמה את סוגי השמע, יצירת המילים, הדקדוק בהקשר או הכתיבה; הפרק האחרון בסימולציה הוא תרגול חלופי בלבד.',
  },
];
...
export const METHODS_INTRO =
  ...
  'כשמבינים את היתרונות והחסרונות של כל גישה, אפשר להתאים את הטקטיקה לקצב הקריאה שלך, במקום לאמץ שיטה בלי לבדוק אם היא מתאימה לך.';
...
export const MARKET_METHODS: { title: string; who: string; fit: string; tradeoff: string; recommended?: boolean }[] = [
  ...
  },
  {
    ...
    tradeoff: 'חיסרון: בלי כיוון מוקדם קוראים בלי לדעת מה מחפשים, וחוזרים לקטע יותר פעמים.',
  },
  {
    title: 'המשולב: הצצה קצרה ואז קריאה מלאה',
    who: 'ההמלצה שלנו',
...
export const HABITS_INTRO =
  'כל השיטות שראית עד כה (מילות קישור, שיטות עבודה לפי סוג, תזמון) הן מיומנויות, ומיומנות נבנית בחזרות, לא בפעם אחת. ארבעת ההרגלים הבאים לא מחליפים את הידע האסטרטגי, אלא הופכים אותו לאוטומטי, כך שביום המבחן אתה לא צריך "לחשוב" על השיטה. אתה פשוט מיישם אותה.';

export const HABITS: { icon: LucideIcon; text: string; href?: string; cta?: string }[] = [
  { icon: BookOpen, text: '20 דקות קריאה באנגלית כל יום, בטקסט לא קל מדי ולא קשה מדי. זו אחת ההמלצות הנפוצות ביותר בהכנה, כי היא בונה זיהוי דפוסי משפט (כולל מילות קישור) באופן טבעי, בלי לשנן בכוח.' },
  { icon: Link2, text: 'שינון מילות קישור: הן מופיעות בכל שלושת סוגי השאלות, ולכן הן ה"מכפיל כוח" היחיד שמשפר בבת אחת גם השלמת משפטים, גם ניסוח מחדש, וגם הבנת הנקרא. תרגל את חבילת 208 המחברים באוצר המילים.', href: '/vocabulary?pos=connector', cta: 'לתרגול המחברים' },
