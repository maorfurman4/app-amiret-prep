      ...
      method: {
        ...
        steps: [
          ...
          },
          {
            ...
            body: 'לא "איזה סוג מילה" — זה כמעט תמיד כבר נתון, כי כל האפשרויות מאותו סוג — אלא "איזה תפקיד": פעולה של תיקון? תיאור של כמות? תוצאה שלילית? ככל שההגדרה שלך חדה יותר, כך קל יותר לפסול אפשרות שנשמעת טוב אבל עושה משהו אחר.',
          },
          {
            title: 'סלק לפי משמעות',
            body: 'הצב כל אפשרות בתוך המשפט, לא לבד. מילה יכולה להיות תקינה כשלעצמה ועדיין לא להתאים להקשר הזה — לכן בודקים תמיד בתוך המשפט המלא. שים לב גם למבנה שאחרי הפער: יש מילים ש"מתחברות" רק למילת יחס מסוימת.',
          },
          {
      ...
      traps: {
        ...
        items: [
          ...
          },
          {
            ...
            detail: 'כמעט תמיד יש אפשרות שאומרת את ההפך המדויק מהמקור. אם נפלת עליה, זה סימן ברור שהבנת נכון את הנושא של המשפט אבל טעית בכיוון היחס הלוגי בתוכו — שווה לחזור ולבדוק מילת קישור.',
          },
          {
  ...
  },
  {
    ...
    workedExample: {
      ...
      walkthrough: [
        ...
        'תשובה 3 — "banned" (נאסר) לא מופיע בקטע בכלל: זה פרט מומצא, לא מידע מהטקסט. גם תשובה 4 — "only for honey" — לא מופיעה בקטע.',
    ...
    deep: {
      ...
      method: {
        ...
        intro:
          'שתי גישות קיצוניות נפוצות בשוק ההכנה: לקרוא את כל השאלות לפני הקטע (מסוכן — הופך את הקריאה ל"חיפוש" ומפספס את הרעיון המרכזי), או לקרוא את הקטע במלואו בלי שום כיוון (מסוכן אחרת — קוראים בלי לדעת מה מחפשים, וחוזרים לטקסט שוב ושוב). ' +
...
export const CONNECTOR_CATEGORIES: ConnectorCategory[] = [
  ...
  },
  {
    ...
    color: 'green',
    intro:
      'מילות תוספת הן הכי "בטוחות" מבחינה לוגית — הן פשוט ממשיכות באותו כיוון שכבר התחיל. כלל הזהב: אם המשפט הראשון היה שלילי, המשך עם "moreover" יהיה גם הוא שלילי (לא הפוך). ' +
